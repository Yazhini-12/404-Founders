...
export function EmployeeLayout() {
  ...
  const navItems = [
    ...
    { name: 'Experience', path: '/employee/experience', icon: Briefcase },
    { name: 'Projects', path: '/employee/projects', icon: FolderKanban },
    { name: 'Work History', path: '/employee/work-history', icon: History },
    { name: 'Learning Hub', path: '/employee/learning', icon: GraduationCap }, // Renamed Learning Hub
    { name: 'Skill Passport', path: '/employee/skills', icon: Award },
    { name: 'Opportunities', path: '/employee/opportunities', icon: Compass },
    { name: 'Skill Gap', path: '/employee/skill-gap', icon: Target },
    { name: 'Career Roadmap', path: '/employee/career-roadmap', icon: MapPin },
    { name: 'AI Career Assistant', path: '/employee/assistant', icon: MessageSquare },
  ...
  const getPageTitle = () => {
    ...
    return item ? item.name : 'Employee Portal';
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <aside className="hidden lg:flex lg:flex-col w-64 bg-slate-900 text-white shrink-0 border-r border-slate-800">
        <div className="p-5 flex items-center gap-3 border-b border-slate-800">
          ...
          <div>
            ...
            <span className="block text-[10px] text-indigo-400 font-semibold uppercase tracking-widest">Employee Portal</span>
          </div>
        </div>
      ...
      {/* Mobile Drawer */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white transform transition-transform duration-200 ease-in-out lg:hidden flex flex-col ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.badge > 0 && (
                  <span className="bg-rose-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">{item.badge}</span>
                )}
              </NavLink>
            );
          })}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="bg-white border-b border-slate-200/80 px-4 sm:px-8 py-3.5 flex items-center justify-between sticky top-0 z-30 shadow-2xs">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* Notification bell */}
            <NavLink to="/employee/notifications" className="relative p-2 text-slate-500 hover:text-slate-900 rounded-lg hover:bg-slate-100 transition-colors">
            ...
            <div className="flex items-center gap-2.5 pl-3 border-l border-slate-200">
              <Avatar name={user?.fullName || 'Ananya R'} src={user?.avatarUrl} size="sm" />
              <div className="hidden md:block text-left">
                <span className="block text-xs font-bold text-slate-900">{user?.fullName || 'Ananya R'}</span>
                <span className="block text-[11px] text-slate-500 font-medium">{user?.jobTitle || 'Backend Developer'}</span>
              </div>
            </div>
          </div>
        </header>
